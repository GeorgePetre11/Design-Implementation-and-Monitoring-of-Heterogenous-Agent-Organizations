...
echo "  Financial Analyst  : ${FINANCIAL_ANALYST_MODEL:-qwen3:14b}"
echo "  Risk Analyst       : ${RISK_ANALYST_MODEL:-qwen3:14b}"
echo "  Strategy Consultant: ${STRATEGY_CONSULTANT_MODEL:-qwen3.5:27b}"
echo "  Evaluator          : Claude Opus 4.6 (API)"

# Check Anthropic API key
if [ -z "${ANTHROPIC_API_KEY}" ]; then
    echo "[level4] WARNING: ANTHROPIC_API_KEY not set — Evaluator will be skipped"
else
    echo "[level4] Anthropic API key configured — Evaluator enabled"
fi
